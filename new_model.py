...
def mlp():
    inputs = tf.keras.layers.Input(shape = (arg.model_input*4), name = 'Input')

    x = tf.keras.layers.Dense(16, activation = 'relu', name = 'd1')(inputs)
    x = tf.keras.layers.Dense(12, activation = 'relu', name = 'd2')(x)
    x = tf.keras.layers.Dense(8, activation = 'relu', name = 'd3')(x)
    x = tf.keras.layers.Dense(5, activation = 'relu', name = 'd4')(x)
    x = tf.keras.layers.Reshape(target_shape=(-1,1,1,1))(x)
    
    model = tf.keras.Model(inputs = inputs, outputs = x, name = 'MLP')
...
def arrow_of_time(name = None):
    """
    Define whether it's normal or abnormal at Arrow of Time task
    """
...
def middle_prediction():
    """
    Define whether it's normal or abnormal at Middle frame Prediction task
    """
    inputs = tf.keras.layers.Input(shape = (arg.flow_size[0]//(2**(arg.front_pools.count(True)+arg.back_channel_pools.count(True))),arg.flow_size[0]//(2**(arg.front_pools.count(True)+arg.back_channel_pools.count(True))), arg.back_filters[-1]))
...
def model():
    ...
    MOI_ENCODED = tf.keras.layers.Multiply()([MOI_front_result, MOI_mlp_result])
    MOI_ENCODED = back_model(MOI_ENCODED)
    ...
    MP_ENCODED = tf.keras.layers.Multiply()([MP_front_result, MP_mlp_result])
    MP_ENCODED = back_model(MP_ENCODED)
    ...
    model = tf.keras.Model(inputs = [AOT_front_input, AOT_mlp_input, MOI_front_input, MOI_mlp_input, MP_front_input, MP_mlp_input], outputs = [aot_result, motion_result, mid_pre_result], name = '3DNN')
...
def MP_model():
    ...
    model = tf.keras.Model(inputs = [MP_front_input, MP_mlp_input], outputs = [mid_pre_result], name = '3DNN')

    return model



def mp_train_test_model():
    # Inputs
    AOT_front_input = tf.keras.layers.Input(shape=(arg.model_input, arg.flow_size[0], arg.flow_size[1], 5), name = 'AOT_Front_Input')
    AOT_mlp_input = tf.keras.layers.Input(shape=(arg.model_input * 4), name = 'AOT_MLP_Input')
